...
| Asset | Size | Best for |
|---|---|---|
| **`WhisperTranscriberSuite-Installer-Windows-….exe`** | ~215 MB | **Most people.** A normal installer: Start-menu shortcut, upgrades in place over an older version, files visible on disk. |
| **`WhisperTranscriberSuite-Portable-Windows-….zip`** | ~330 MB | Unzip and run. No installation, no admin rights, happy on a USB stick. |
...
so you just download the newer `WhisperTranscriberSuite-Installer-Windows-….exe` and run it — it upgrades

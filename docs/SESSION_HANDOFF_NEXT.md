...
- **TODO for the next build round (owner, 2026-09-23): fix these.** Open app/test issues found (not
  fixed in v1.9.0 — app code): see "Open issues" in
...
|---|---|---|---|---|
| Auto-grey incompatible hardware combos | `bt73j12pu` | merged into `master` | `opencode/hardware-greying` (merged) | **MERGED INTO MASTER, 2026-09-21.** The real review this row asked for happened: an independent Sonnet sub-agent found one genuine bug (Advanced dialog's readiness warning / combobox marker read the last-saved config instead of this dialog's own live, unsaved credential fields — a just-typed working key still showed "unavailable" until Save). Fixed with a regression test (proven to fail on pre-fix code, pass after), pyright 0/0/0 + full suite green re-confirmed on the merged result, then fast-forwarded into `master` and pushed. Nothing left pending on this item. |
...
| Adversarial review + fix pass on `core/search.py`, `core/chapters.py`, `core/_errors.py`, `core/_proc.py`, `core/_threads.py`, `core/paths.py`, `core/logging_setup.py` | ~~`befdou9df`~~ killed with zero progress → relaunched as `b311mm7mt` | `C:\Users\Owner\Desktop\whisper_app\wt-search-chapters-infra\` (worktree) | `opencode/search-chapters-infra-review` | **DONE, best-verified of today's batch.** 3 commits (`cb7e07a` fix, `d4adf4e` handoff, `9340a74` independent re-verification — a genuine oddity: the ORIGINALLY-killed `befdou9df` process turned out not to be fully dead and finished on its own while the relaunch was starting; the relaunch found this, and instead of duplicating, independently re-derived and empirically proved every fix by reverting to pre-fix code and showing 8/9 new tests fail, then restoring and showing them pass). 6 real bugs fixed in `core/search.py` (FTS score always 1.0, multi-word queries treated as exact phrase, a transient read error silently wiping a file's search index, semantic-search exceptions breaking the documented FTS fallback, embeddings never backfilled after an FTS-only index, one bad transcript aborting a whole reindex walk) + 2 hardening improvements (`_proc.py` refusing to signal the app's own process group, `logging_setup.py` pruning unbounded per-worker log growth). `chapters.py`/`_errors.py`/`_threads.py`/`paths.py` read closely, genuinely clean. Full details + proofs: `OPENCODE_HANDOFF_search_chapters_infra.md` on that branch. Author identity correct (`Milomilo777`) throughout — this ran entirely after the git-config fix. |

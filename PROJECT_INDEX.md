...
- `app/domain/cookies.py` — Browser-cookie option shared by the Download tab picker, Advanced settings and both yt-dlp services: dropdown choices (Safari on macOS), `--cookies-from-browser` args with browser validation, cookie-jar-failure detection (retry without cookies) and login-wall hints.
...
- `app/dialogs/advanced.py` — AdvancedDialog: the single settings hub — model + engine pickers (faster-whisper/whisper.cpp/Gemini/Google Cloud STT/NVIDIA Parakeet) plus the model folder, output-format checkboxes, silence/noise handling (VAD, denoise, Demucs, hallucination flagging), prompt/hotwords, AI Layer, watched folder, SponsorBlock categories, hardware-wizard + model-download launchers. Each engine's own setup section — and the remote-LLM fields — is packed only while that engine/provider is picked (`_sync_engine_sections` / `_sync_llm_provider_rows`); one `_build_*_section` method per section; the Gemini and Google Cloud setups sit in collapsed, greyed `collapsible_section`s and the VAD sliders behind "Fine-tune". ~2200 lines.
...
- `app/dialogs/model_advisor.py` — ModelAdvisorDialog ("Best for this PC…" on the Transcribe tab): runs `hardware.cuda_status()` + `recommend_models()` on a daemon thread and offers the fastest / most accurate model with a "Use this model" button.
...
- `core/hardware.py` — Tier auto-probe (CUDA float16/int8_float16 -> QNN NPU -> OpenVINO -> DirectML -> CPU int8), hardware.json persistence, and detect_device_for() -- the single canonical (device, compute_type) resolver used by transcriber.py and backends/faster_whisper_be.py. `cuda_status()` checks the whole CUDA chain (driver API, CTranslate2 `get_cuda_device_count()`, compute types, cuBLAS discovery/preload from NVIDIA pip wheels / CUDA Toolkit) and says what is broken; `warm_up_cuda_model()` runs one GPU pass at load; `recommend_models()` backs the model advisor; `python -m core.hardware` prints a diagnostics report.
- `core/js_runtime.py` — Deno for yt-dlp's YouTube JS challenges: finds a bundled/app-installed Deno, builds `--js-runtimes deno:<path>` (only for yt-dlp >= 2025.11.12), installs Deno on demand (official zip, SHA-256 verified, user cache), and the "install / update yt-dlp" hint.
